...
def optimizeU(n,cost,tmax,numGen):
    ...
    uncon_prob = pg.decompose(prob = my_prob,method = "tchebycheff", weight = [.25,.25,.25,.25],z = [sum(cost)/3,B_met[0],B_met[1],B_met[2]])
...
def writeLogU(n,cost,pop,B_met,tmax,uda,totalTime):
    ...
    riskZ = pop.get_f()[pop.best_idx()][1] 
    timeZ = -(pop.get_f()[pop.best_idx()][2])
    timeZ = ">"+str(tmax) if timeZ ==  tmax+1 else timeZ #if does not go extinct in tmax
    abundZ = -(pop.get_f()[pop.best_idx()][3])
