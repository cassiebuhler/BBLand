...
getBatchFile <- function(n,seed,contPath,binPath,path)
{
  ...
  datPath <- paste("/DAT=",contPath,sep = "")
  ...
  line2<- paste('START /WAIT "title"',RAMAS_spatial,contPTCFile,'/RUN=YES /TEX',datPath)
  line3<- paste('START /WAIT "title"',RAMAS_hab,bothPDYfile,'/RUN=YES /TEX')
  line4<- paste('START /WAIT "title"',RAMAS_spatial,bothMPfile,'/RUN=YES /TEX')
